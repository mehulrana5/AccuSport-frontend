    ...
    return (
        <nav className="nav-bar">
            ...
            <ul className="nav-links">
                <li><Link className="nav-link" to="/organization">Organization</Link></li>
                <li><Link className="nav-link" to="/tournaments">Tournaments</Link></li>
                <li><Link className="nav-link" to="/matches">Matches</Link></li>
                <li><Link className="nav-link" to="/teams">Teams</Link></li>
                <li><Link className="nav-link" to="/players">Players</Link></li>
                {context.userInfo.roles.includes('player') ?
                    (
                        !context?.playerInfo?.user_id ? <li><Link className="nav-link" to={`/createPlayer`}>Create player</Link></li> :
                            <>
                                <li><Link className="nav-link" to={`/player/update/${context?.playerInfo?._id}`}>My profile</Link></li>
                                <li><Link className="nav-link" onClick={toggleNotificationModal}>Notifications</Link></li>
                            </>
                    ):<></>
                }
                {!context.userInfo.roles.includes('guest') ?
                    <>
                        <li><Link className="nav-link" to="/login">Login</Link></li>
                        <li><Link className="nav-link" to="/signup">Signup</Link></li>
                    </>
                    : <li><Link onClick={handleLogout} className="nav-link" to="/">Logout</Link></li>
                }
                {
